...
UNIT
  ...
  echo "发布包准备完成： $DEPLOY_DIR"
  echo "可使用 tar 打包： tar -czf chat-community-deploy.tar.gz -C $ROOT_DIR deploy_package"
}
...
function generate_env_file() {
  ...
  if [ -n "$existing_pw" ]; then
    ...
    if command -v openssl >/dev/null 2>&1; then
      ...
      PG_PASSWORD=$(head -c 24 /dev/urandom | base64)
    fi
  fi

  JWT_SECRET=${JWT_SECRET:-$(openssl rand -hex 32 2>/dev/null || head -c 32 /dev/urandom | base64)}
  SESSION_SECRET=${SESSION_SECRET:-$(openssl rand -hex 32 2>/dev/null || head -c 32 /dev/urandom | base64)}
...
echo "完成。后续建议："
echo " 1) 将 $DEPLOY_DIR 打包并传到目标 Linux 服务器： tar -czf chat-community-deploy.tar.gz deploy_package"
echo " 2) 在目标机器解压并进入 deploy_package，运行 ./install_prod_deps.sh 安装生产依赖并使用 ./run_api.sh 启动。"
echo " 3) 可参考 chat-community.service 模板部署为 systemd 服务（修改 WorkingDirectory 和 ExecStart 路径）。"
