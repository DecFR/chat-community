...
if (enableCluster && cluster.isPrimary) {
  ...
  cluster.on('exit', (worker) => {
    logger.warn(`⚠️ Worker ${worker.process.pid} 退出，自动重启...`);
    cluster.fork();
  });
} else {
  // 🚀 关键修复：确保 chunk-upload 路由被包含在预加载列表中
  Promise.all([
    import('express'),
    import('http'),
    import('cors'),
    ...
    import('./routes/chunk-upload.routes.js'), // <--- 必须在这里
    ...
    .then(
      ([
        ...
        { default: chunkUploadRoutes }, // <--- 解构出来
        ...
        if (!fs.existsSync(absoluteUploadPath)) {
          try {
            ...
            console.log(`✅ Upload directory created: ${absoluteUploadPath}`);
          } catch (error) {
        ...
        app.use('/uploads', cors({ origin: '*' }), express.static(uploadsPath));

        // --- 路由注册 (顺序非常重要) ---
        app.use('/api/auth', authRoutes);

        // 🚀 修复 404：在其他 API 之前注册 chunk-upload
